...
int main(int argc, char** argv)
{
    ...
    const char* xmlfile = "../input_3_bus_phase_B.xml";
    std::vector<std::string> timestamps = build_timestamps_24h();

    {
        std::ofstream clr("gpk_timeseries_phaseB.csv");
    }
    ...
    for (std::size_t k = 0; k < timestamps.size(); ++k) {
        ...
        if (k == 0) {
            app.writeBusVoltagesCSV("bus_voltages_phaseB_standalone.csv");
        }

        app.appendBusVoltagesTimeSeriesCSV("gpk_timeseries_phaseB.csv", timestamps[k], "B");

        double V = app.getBoundaryMagnitude();
        double P = app.getBoundaryPinjection();

        std::cout << "[Phase B][" << timestamps[k]
            << "] Boundary V = " << V
            << " pu, P = " << P << std::endl;
    }
